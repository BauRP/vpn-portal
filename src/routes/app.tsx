...
import { VpnProvider, useVpn, formatUptime } from "@/components/mastervpn/VpnContext";
...
export const Route = createFileRoute("/app")({
  head: () => ({
    meta: [
      { title: "Trivo VPN — Live Console" },
      {
        name: "description",
        content:
          "Trivo VPN live console: connect, switch servers, monitor bandwidth and tunnel health.",
      },
    ],
  }),
  component: AppRoute,
...
const tabs: { to: "/app" | "/app/settings" | "/app/profile"; key: keyof TranslationKeys; icon: string }[] = [
      ...
      <div className="relative mx-auto flex h-[100dvh] w-full max-w-[480px] flex-col overflow-hidden bg-background">
        ...
        {killSwitchTriggered && (
          <div role="alert" className="flex shrink-0 items-start gap-2 border-b border-destructive/40 bg-destructive/10 px-5 py-2.5">
            ...
            <button
              onClick={clearKillSwitchTriggered}
              className="font-mono text-[10px] uppercase tracking-widest text-destructive hover:text-destructive/70"
            >
        ...
        <nav
          className="absolute inset-x-0 bottom-0 grid grid-cols-3 border-t border-border bg-black"
